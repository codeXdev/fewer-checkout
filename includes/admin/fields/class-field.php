...
namespace FewerWC\Admin\Fields;
...
abstract class Field {
	...
	protected function maybe_render_timestamp() {
		...
		$fewerwc_timestamps = \get_option( FEWERWC_SETTINGS_TIMESTAMPS, array() );

		if ( empty( $fewerwc_timestamps ) || empty( $fewerwc_timestamps[ $this->args['name'] ] ) ) {
			return;
		}

		$timestamp           = $fewerwc_timestamps[ $this->args['name'] ];
		...
		?>
		<p class="description fewerwc-setting-timestamp"><small><em><?php esc_html_e( 'Last updated', 'fewer' ); ?>: <?php echo esc_html( $formatted_timestamp ); ?></em></small></p>
		<?php
	}
}
