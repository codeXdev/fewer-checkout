...
/**
 * Fewer Checkout payment gateway.
 *
 * @extends WC_Payment_Gateway
 */
class WC_Gateway_Fewer extends \WC_Payment_Gateway {

    /**
     * Fewer disclaimer text.
     *
     * @var string
     */
    protected $fewer_disclaimer = '';

    /**
     * Gateway class constructor.
     */
    public function __construct() {
        $this->fewer_disclaimer = __( 'Note: Fewer Checkout will continue to function even if it is disabled as a payment gateway. This option is here for integrations that require an active payment gateway code.', 'fewercheckout' );

        $this->id                 = FEWERWC_PAYMENT_METHOD_ID;
        ...
        $this->init_settings();
        $this->method_title       = FEWERWC_PAYMENT_METHOD_SLUG;
        $this->method_description = sprintf(
            '%1$s <a href="%2$s">%3$s</a>.<br />%4$s',
            __( 'Payment through', 'fewercheckout' ),
            esc_url( admin_url( 'admin.php?page=fewer' ) ),
            __( 'Fewer Checkout', 'fewercheckout' ),
            esc_html( $this->fewer_disclaimer )
        );


        // Load the settings.
        $this->enabled = $this->get_option( 'enabled' );
        $this->description = __( 'Pay securely with MADA, credit cards or with Apple Pay', 'fewercheckout' );
        $this->title   = __( 'Fewer Checkout', 'fewercheckout' );
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                'title'       => __( 'Enable/Disable', 'fewercheckout' ),
                'type'        => 'checkbox',
                'label'       => __( 'Enable Fewer', 'fewercheckout' ),
                'description' => $this->fewer_disclaimer,
                'default'     => 'yes',
            ),
        );
    }
    public function set_icons($icon, $id): string
    {
        ...
        if ($id == $this->id) {
            $img = FEWERWC_PLUGIN_DIR . '/assets/images/logos.png';
            return "<img style='padding:2px ; max-height: 1.618em;' src='$img' >";
        }
        return $icon;
    }
...
//        }
    ...
    function process_refund($order_id, $amount = null, $reason = ''): bool
    {
        ...
        return fewerwc_refund_request($order, $request);
    }

    function receipt_page($order_id)
    {
        fewerwc_render_cart(FEWERWC_BTNPOS_BEFORE_CHECKOUT, $order_id);
    }
}
